...
from magiparser.magiparse_shorthand import *
...
class FuncHandler(ResultsHandler):
    ...
    def assertlen(self,n):
        # This function makes sure that the number of arguments in the .mgi file patch the expected number of arguments for the specified function
    ...
    def GenerateOutputGeneral(self,linebreaks,args):
        ...
        for arg in args:
            if(isinstance(arg,tuple)):
                ...
                out += "\n"+" "*(self.depth+config.defaultdepthdelta)+"db "
                comma = ""
                continue
            elif(arg == "dw"):
                out += "\n"+" "*(self.depth+config.defaultdepthdelta)+"dw "
                comma = ""
                continue
            elif(isinstance(arg,str)):
                out += "\n"+" "*(self.depth+config.defaultdepthdelta)+"{}".format(arg)
                comma = " "
                continue
            else:
        ...
        # Add the line breaks at the beginning of the output
        # Add the comment with the address
        endoffirstline = out.find("\n")
        if(endoffirstline == -1): #Not found, i.e. single-line command
            endoffirstline = len(out)
        commentspacing = config.commentoffset - min(endoffirstline,config.commentoffset-1)
        out = "\n"*linebreaks + out[:endoffirstline] + " "*commentspacing + "; ${:04X}".format(self.address) + out[endoffirstline:]
        return out
